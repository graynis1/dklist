import { getClubBySlug, joinClub } from "@/db/queries/book-clubs";
...
export async function POST(request: Request, { params }: { params: Promise<{ slug: string }> }) {
  ...
  try {
    const result = await joinClub(club.id, session.userId);
    return mobileJson({ status: "ok", ...result });
  } catch (error) {
    const message = error instanceof Error ? error.message : "Katılınamadı.";
