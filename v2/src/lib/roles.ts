/**
 * Pure role data/logic with zero server dependency - deliberately split out
 * of permission.ts (which has `import "server-only"` because of its
 * auth()-calling requireRole()) after a real build failure: a client
 * component (user-admin-row.tsx, the role-select dropdown) importing
 * USER_TYPES/ROLE_LABELS straight from permission.ts pulled `server-only` -
 * db/index.ts - mysql2 into the client bundle, the exact same failure class
 * PLAN.md already documents for reading-status.ts. permission.ts re-exports
 * everything here so none of its existing (server-only) importers need to
 * change.
 */

/**
 * v1's UserTypeEnum (`SuperAdmin, Admin, Mod, Blog_Yazari, Üye`) plus the
 * customer's new-for-v2 role hierarchy ask: `Kurucu` (founder - can be more
 * than one person, but once granted must be UN-REVOCABLE by anyone, stated
 * twice for emphasis in the customer's notes), and `Yazar`/`Yayinevi`
 * (author/publisher members who can submit book data directly - addresses
 * the "only admin can add data" bottleneck the customer flagged). `Mod`'s
 * stored value is unchanged (no data migration needed - it's a free-text
 * varchar) but its public-facing label is "Kütüphaneci" everywhere in the
 * UI per the customer's explicit ask - see ROLE_LABELS below.
 */
