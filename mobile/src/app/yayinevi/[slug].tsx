...
import { getPublisher, togglePublisherLike, type EntityDetail, type EntityBookItem } from "@/api/entity";

export default function PublisherScreen() {
  const { colors, spacing } = useTheme();
  const { slug } = useLocalSearchParams<{ slug: string }>();
  const [publisher, setPublisher] = useState<EntityDetail | null>(null);
  ...
  async function onToggleLike() {
    setSaving(true);
    try {
      const result = await togglePublisherLike(slug);
      setLiked(result.liked);
  ...
  if (!publisher) {
    return (
      <View style={{ flex: 1, alignItems: "center", justifyContent: "center", backgroundColor: colors.bg, padding: spacing["2xl"] }}>
        <ThemedText variant="body" muted>Yayınevi bulunamadı.</ThemedText>
      </View>
    );
  }
  ...
  return (
    <ScrollView style={{ flex: 1, backgroundColor: colors.bg }} contentContainerStyle={{ padding: spacing.lg, gap: spacing.lg }}>
      <View style={{ gap: spacing.xs }}>
        <ThemedText variant="headline">{publisher.name}</ThemedText>
        <ThemedText variant="caption" muted>{likeCount} takipçi</ThemedText>
      </View>

      <Button title={liked ? "Takip Ediliyor ✓" : "Takip Et"} variant={liked ? "primary" : "secondary"} onPress={onToggleLike} disabled={saving} block />
